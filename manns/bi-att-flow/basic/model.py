...
class Model(object):
    ...
    def _build_forward(self):
        ...
        num_templates = 8
        with tf.variable_scope("templates"), tf.device("/cpu:0"):
            templates_mat = tf.get_variable("templates_mat", shape=[N, num_templates, d], dtype='float')

        # passage  are batch_size, max_num_sents, max_sent_size, hidden_size (fw + bw is 2*hs)
        # question are batch_size, max_ques_size, hidden_size                (fw + bw is 2*hs)
        # template are batch_size, num_templates, hidden_size

        # we want to express each hidden size as a lin-comb

        def templify(vectors, passage=True):
            if passage:
                # this means we are processing the passage
                similarities = tf.einsum('ijk,ilmk->ilmj', templates_mat, vectors)
                similarities = tf.nn.softmax(similarities)
                new_hidden = tf.einsum('ilmj,ijk->ilmk', similarities, templates_mat)
            else:
                # processing the question
                similarities = tf.einsum('ijk,ilk->ilj', templates_mat, vectors)
                similarities = tf.nn.softmax(similarities)
                new_hidden = tf.einsum('ilj,ijk->ilk', similarities, templates_mat)
            return new_hidden

        fw_u, bw_u = templify(fw_u, False), templify(bw_u, False)
        fw_h, bw_h = templify(fw_h), templify(bw_h)

        self.tensor_dict['templates'] = templates_mat
        u = tf.concat(axis=2, values=[fw_u, bw_u])
        h = tf.concat(axis=3, values=[fw_h, bw_h])  # [N, M, JX, 2d]
        self.tensor_dict['u'], self.tensor_dict['h'] = u, h
    ...
    def _build_loss(self):
        ...
        if config.wy:
            ...
            templates_reg = .01
            templates_loss = templates_reg * tf.norm(self.tensor_dict['templates'])
            tf.add_to_collection('losses', templates_loss)
